...
class LegalEntityContractDocPDF(BaseLegalEntityDocsPDF):
    ...
    def get_pdf(self):
        ...
        add_doc_data2 = [
            [
                Paragraph(f'г. Город', self.BOLD_STYLE),
                Paragraph(
        ...
        elements.append(
            Paragraph(
                f'<b>{self.legal_entity_contract.legal_entity.short_name}</b>, Именуемый в дальнейшем "ЗАКАЗЧИК", в лице Директора: {self.legal_entity_contract.legal_entity.director_name}, действующего на основании Устава, и <b>Индивидуальный Предприниматель Директор Директорович</b>, именуемый в дальшейшев "ИСПОЛНИТЕЛЬ", действующий на основании Листа записи в ЕГРИП о гусдарственной регистрации физического лица в качестве индивидуального предпринимателя от 23 сентября 2021 года, заключили договор о нижеследующем:',
                self.NORMAL_STYLE,
            )
        )
